...
const Forgot = () => {
    ...
    return (
        <Container onSubmit={handleSubmit}>
            ...
            {success ? <p style={{ textAlign: 'center' }}>Нууц үг сэргээх линкийг таны и-мэйл хаягруу илгээлээ.</p> : <>
                <input required value={email} onChange={(e) => setEmail(e.target.value)} type="text" placeholder="И-мэйл хаяг" name="email" />
                <button type="submit">Илгээх</button>
            </>}
            <div className='or' style={{ marginTop: 5, marginBottom: 5, opacity: 0.7, fontSize: 10 }}>&nbsp;</div>
            <button type="button" onClick={() => navigate('/login')}>Нэвтрэх</button>
        </Container>
    );
};
